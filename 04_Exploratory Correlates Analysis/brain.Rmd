...
class_membership_597$class_lv1_3 <- factor(
  class_membership_597$class_lv1_3, levels = c(1,2,3),
  labels= c("Low","Persistent High","Remitting")
)

class_membership_597$class_lv1_3 <- factor(
  class_membership_597$class_lv1_3,
  levels = c("Low", "Remitting", "Persistent High")
)

classes <- class_membership_597 %>%
  dplyr::select(pin,class_lv1_3)
...
for (r in names(brain)[4:246]) {
  for (c in comparisons){
    dat <- brain %>% filter(class_lv1_3 %in% c)
    # Assign the numeric column and group factor
    x <- dat[[r]]
    g <- factor(dat$class_lv1_3, levels = c(c[1], c[2]))  # relevel
...
##filter results
png("cdss_thickness_effect_sizes.png", width = 1600, height = 1200, res = 200)
ggplot(thick_results, aes(x = label, y = estimate, shape = comparison)) +
...
##filter results
png("cdss_vol_effect_sizes.png", width = 1600, height = 1200, res = 200)
ggplot(vol_results, aes(x = label, y = estimate, shape = comparison)) +
  geom_point(aes(alpha = abs(estimate) >= 0.33), 
             position = position_dodge(width = 0.5), size = 2) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high, alpha = abs(estimate) >= 0.33), 
...
area_results <- area_results %>% arrange(estimate)
...
##filter results
png("cdss_area_effect_sizes.png", width = 1600, height = 1200, res = 200)
ggplot(area_results, aes(x = label, y = estimate, shape = comparison)) +
  geom_point(aes(alpha = abs(estimate) >= 0.33), 
             position = position_dodge(width = 0.5), size = 2) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high, alpha = abs(estimate) >= 0.33), 
